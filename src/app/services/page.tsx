          ...
          >
            {categories.map((cat) => (
              <a
                key={cat.key}
                href={`#${cat.key}`}
                className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium bg-white border border-slate-200 shadow-sm text-slate-700 hover:text-red-primary hover:border-red-primary/50 transition-all flex items-center"
              >
                    ...
                    <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 md:gap-12 items-center mb-8 sm:mb-10">
                      <div className={!isEven ? "lg:order-2" : ""}>
                        <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                          <div
                            className={`w-11 h-11 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br ${cat.color} flex items-center justify-center shrink-0`}
                          >
                            <cat.icon className="text-lg sm:text-2xl text-white" />
                          </div>
                          <div>
                            <h2 className="text-lg sm:text-2xl md:text-3xl font-extrabold text-slate-900 flex items-center gap-2">
                              <span>{cat.emoji}</span> {cat.title}
                            </h2>
                            <span
                              className={`text-xs sm:text-sm font-medium ${cat.accent}`}
                            >
                              {cat.tagline}
                            </span>
                          </div>
                      ...
                      <div className={!isEven ? "lg:order-1" : ""}>
                        <div
                          className={`aspect-[4/3] rounded-2xl sm:rounded-3xl bg-gradient-to-br ${cat.color} p-px overflow-hidden`}
                        >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                      {cat.subServices.map((sub) => {
                        const cardContent = sub.image ? (
                          ...
                          <div className="p-4 sm:p-5">
                            <div
                              className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br ${cat.color} flex items-center justify-center mb-2.5 sm:mb-3`}
                            >
                              <sub.icon className="text-sm sm:text-base text-white" />
                            </div>
                        ...
                        const cardClass = `h-full rounded-xl sm:rounded-2xl bg-white border border-slate-200 shadow-sm ${cat.hoverBorder} hover:shadow-md transition-all group overflow-hidden ${
