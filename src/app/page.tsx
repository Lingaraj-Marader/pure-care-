  ...
  return (
    <>
      ...
      <section className="py-8 sm:py-10 md:py-14 relative overflow-hidden">
        ...
        <div className="relative z-10 w-full px-4 sm:px-6 lg:px-10 xl:px-14 text-center">
          ...
          <motion.p
            ...
            className="text-sm sm:text-lg md:text-xl text-slate-300 max-w-2xl mx-auto mb-6 sm:mb-8 px-2"
          >
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4 sm:px-0">
            <Link
              href="/services"
              className="px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-sky-500 via-blue-600 to-blue-700 hover:from-sky-400 hover:to-blue-600 text-white font-bold rounded-full text-sm sm:text-lg hover:shadow-xl hover:shadow-sky-500/30 transition-all duration-300 pulse-glow flex items-center justify-center gap-2 border border-sky-400/30"
            >
            ...
            <a
              ...
              className="px-6 sm:px-8 py-3 sm:py-4 border-2 border-green-500 text-green-400 font-bold rounded-full text-sm sm:text-lg hover:bg-green-500/10 transition-all duration-300 flex items-center justify-center gap-2 shadow-lg shadow-green-500/10"
            >
          ...
          <div className="mt-10 sm:mt-14 md:mt-16 grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
            {stats.map((s, idx) => (
              <motion.div
                ...
                className="text-center p-4 sm:p-5 rounded-2xl bg-[#0a1638]/70 backdrop-blur-md border border-sky-500/20 shadow-lg shadow-navy-950/60 hover:border-sky-400/40 transition-all"
              >
                <div className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-sky-400 mb-0.5 sm:mb-1 drop-shadow-sm">
                  {s.value}
                </div>
        ...
        <div className="w-full px-4 sm:px-6 lg:px-10 xl:px-14">
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
            <FadeIn direction="left">
              ...
              <Link
                href="/about"
                className="inline-flex items-center gap-2 px-6 sm:px-8 py-2.5 sm:py-3 bg-gradient-to-r from-sky-500 via-blue-600 to-blue-700 hover:from-sky-400 hover:to-blue-600 text-white text-sm sm:text-base font-semibold rounded-full shadow-lg shadow-sky-500/25 hover:shadow-sky-400/40 transition-all duration-300 border border-sky-400/30"
              >
      ...
      <section className="py-8 sm:py-12 md:py-16 relative">
        ...
        <div className="w-full px-4 sm:px-6 lg:px-10 xl:px-14">
            ...
            {homeServices.map((service, idx) => (
              <FadeIn key={service.title} delay={idx * 0.08}>
                <Link href={service.link} className="block h-full">
                  <div className="h-full rounded-xl sm:rounded-2xl bg-[#0a1638]/85 backdrop-blur-md border border-sky-500/20 shadow-lg shadow-navy-950/60 hover:border-sky-400/60 hover:shadow-xl hover:shadow-sky-500/20 transition-all duration-300 group cursor-pointer overflow-hidden">
                    ...
                    <div className="p-3 sm:p-4 md:p-5">
                      <h3 className="text-white font-bold text-xs sm:text-sm md:text-lg mb-1 sm:mb-2 group-hover:text-sky-300 transition-colors">
                        {service.title}
                      </h3>
                      <p className="text-slate-300 text-[10px] sm:text-xs md:text-sm leading-relaxed line-clamp-2 sm:line-clamp-3">
                        {service.desc}
                      </p>
                    </div>
                  </div>
          ...
          <div className="text-center mt-8 sm:mt-12">
            <Link
              href="/services"
              className="inline-flex items-center gap-2 px-6 sm:px-8 py-2.5 sm:py-3 bg-[#0a1638] border border-sky-500/30 hover:border-sky-400 text-sky-300 hover:text-white text-sm sm:text-base font-semibold rounded-full transition-all duration-300 hover:bg-sky-500/20 hover:shadow-lg hover:shadow-sky-500/20"
            >
            ...
            {homeProducts.map((prod, idx) => (
              <FadeIn key={prod.title} delay={idx * 0.08}>
                <Link href="/products" className="block">
                  <div className="relative aspect-[6/5] rounded-xl sm:rounded-2xl overflow-hidden border border-sky-500/20 shadow-lg shadow-navy-950/60 hover:border-sky-400/50 hover:shadow-xl hover:shadow-sky-500/20 transition-all group bg-[#0a1638]">
                    <Image
                      src={prod.image}
                      alt={prod.title}
                      fill
                    ...
                    />
                    {/* Hover Overlay */}
                    <div className="absolute inset-0 bg-[#060c20]/95 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center p-4 sm:p-5">
                      <div
                        className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-gradient-to-br ${prod.color} flex items-center justify-center mb-2 sm:mb-3 shadow-md`}
                      >
                      ...
                      <ul className="space-y-1 sm:space-y-1.5 mb-2 sm:mb-3">
                        {prod.items.map((item) => (
                          ...
                          >
                            <FaCheckCircle className="text-sky-400 shrink-0 text-[8px] sm:text-[10px]" />
                            {item}
                          </li>
                        ))}
                      </ul>
                      <span className="text-sky-400 font-bold text-xs sm:text-sm">
                        {prod.price}
                      </span>
                    </div>
                  </div>
                  <h3 className="text-slate-200 font-bold text-xs sm:text-sm md:text-base mt-2 sm:mt-3 text-center group-hover:text-sky-300 transition-colors">
                    {prod.title}
                  </h3>
                </Link>
              </FadeIn>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
            {whyChoosePreview.map((item, idx) => (
              <FadeIn key={item.title} delay={idx * 0.08}>
                <motion.div
                  whileHover={{ y: -6 }}
                  className="h-full p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl bg-[#0a1638]/85 backdrop-blur-md border border-sky-500/20 shadow-lg shadow-navy-950/60 hover:border-sky-400/50 hover:shadow-xl hover:shadow-sky-500/20 transition-all text-center group"
                >
                  <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-sky-500 to-blue-700 flex items-center justify-center mx-auto mb-3 text-white shadow-md shadow-sky-500/30">
                    <item.icon className="text-base sm:text-lg" />
                  </div>
                  <h3 className="text-white font-bold text-xs sm:text-sm md:text-base mb-1 group-hover:text-sky-300 transition-colors">
                    {item.title}
                  </h3>
      ...
      <section className="py-8 sm:py-12 md:py-16">
        <div className="w-full px-4 sm:px-6 lg:px-10 xl:px-14">
          <FadeIn>
            <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-r from-sky-600 via-blue-700 to-navy-950 text-white p-6 sm:p-10 md:p-14 text-center overflow-hidden shadow-2xl shadow-sky-500/20 border border-sky-400/40">
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_rgba(56,189,248,0.25),_transparent_70%)]" />
              <div className="relative z-10 max-w-2xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold rounded-full text-sm sm:text-lg hover:shadow-xl hover:shadow-green-500/40 transition-all justify-center shadow-lg"
                  >
                    <FaWhatsapp className="text-lg sm:text-xl" /> WhatsApp Us
                  </a>
                  <Link
                    href="/contact"
                    className="inline-flex items-center gap-2 px-6 sm:px-8 py-3 sm:py-4 bg-white/10 hover:bg-white text-white hover:text-navy-950 border border-white/30 font-bold rounded-full text-sm sm:text-lg hover:shadow-xl transition-all justify-center backdrop-blur-sm"
                  >
